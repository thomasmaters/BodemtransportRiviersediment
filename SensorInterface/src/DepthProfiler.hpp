...
class DepthProfiler
{
  ...
  public:
    ...
    {
    	auto akaas = ((1/4)*params.at(0,0)*std::pow(start,4)) + ((1/3)*params.at(1,0)*std::pow(start,3)) + ((1/2)*params.at(2,0)*std::pow(start,2)) + params.at(3,0)*start;
    	auto bkaas = ((1/4)*params.at(0,0)*std::pow(end,4)) + ((1/3)*params.at(1,0)*std::pow(end,3)) + ((1/2)*params.at(2,0)*std::pow(end,2)) + params.at(3,0)*end;

    	return std::abs(akaas-bkaas);
    }
    ...
    void addProcessedPoint(Matrix<N, 3, T>& matrix)
    {
        try
        {
            ...
            for (std::size_t i = 0; i < waves.size(); ++i) {
                ...
                if(dune.size_ > 60) {continue;}
                if(dune.start_ + 60 > N){  return;}
                ...
                for (std::size_t i = 0; i < dune.size_; ++i) {
    ...
    template <std::size_t H, std::size_t W>
    std::vector<std::pair<std::size_t, std::size_t>> classifyWave(const Matrix<H, W, T>& afgeleide,
