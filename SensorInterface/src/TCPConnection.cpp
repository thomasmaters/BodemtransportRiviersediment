...
namespace Communication::TCP
{
    ...
    {
        boost::asio::async_read(getSocket(), boost::asio::buffer(data_, 5),
    ...
    void TCPSession::handleOutgoingConnection(const boost::asio::mutable_buffer& message, Type response_indentifier, bool has_response_head_and_body, std::shared_ptr<ResponseHandler> response_handler)
    {
        std::cout << "STARTED SENDING REQUEST: " << (char*)message.data() << std::endl;
        boost::asio::async_write(getSocket(), boost::asio::buffer(message),
                                 boost::bind(&TCPSession::handleResponse<Type>, shared_from_this(), response_indentifier,
                                		 has_response_head_and_body, boost::asio::placeholders::error,
                                             boost::asio::placeholders::bytes_transferred, response_handler));
    }

    template <typename Type>
    void TCPSession::handleResponse(Type response_indentifier, bool has_response_head_and_body, const boost::system::error_code& error, std::size_t bytes_transferd, std::shared_ptr<ResponseHandler> response_handler)
    {
        ...
        if (!error)
        {
            ...
            if
                ...
                {
                    boost::asio::async_read_until(getSocket(), aBuffer, response_indentifier,
            ...
            {
                boost::asio::async_read(getSocket(), boost::asio::buffer(data_, response_indentifier),
                                        boost::bind(&TCPSession::handleReceivedResponse, shared_from_this(),
                                        		has_response_head_and_body, boost::asio::placeholders::error,
                                                    boost::asio::placeholders::bytes_transferred, response_handler));
            }
    ...
    void TCPSession::handleReceivedResponse(bool has_response_head_and_body, const boost::system::error_code& error, std::size_t bytes_transferd, std::shared_ptr<ResponseHandler> response_handler)
{
    ...
    void TCPSession::handleRequest(const boost::system::error_code& error, std::size_t bytes_transferd, std::shared_ptr<RequestHandler> request_handler)
{
        if (!error || error == boost::asio::error::eof)
        {
            ...
            boost::asio::async_write(getSocket(), boost::asio::buffer(data_, 5),
	...
	TCPServerClient::TCPServerClient(boost::asio::io_service& io_service, std::string host, std::string remote_port, std::string local_port)
      : host_(host)
    ...
    void TCPServerClient::sendRequest(const SensorMessage& message, std::size_t response_size, bool has_response_head_and_body)
    {
        sendMessage(boost::asio::mutable_buffer(message.getData(), message.getDataLength()), response_size, has_response_head_and_body);
    }
    ...
    void TCPServerClient::start_accept()
    {
        ...
        acceptor_.async_accept(session->getSocket(), boost::bind(&TCPServerClient::handle_accept, this, session, boost::asio::placeholders::error));
    }
    ...
    template <typename Type>
    void TCPServerClient::sendMessage(const boost::asio::mutable_buffer& message_buffer, Type response_indentifier, bool has_response_head_and_body)
    {
    ...
    template <typename Type>
    void TCPServerClient::handleConnect(std::shared_ptr<TCPSession> session, const boost::asio::mutable_buffer& message_buffer, Type response_indentifier, bool has_response_head_and_body, const boost::system::error_code& error)
    {
        if (!error)
        {
