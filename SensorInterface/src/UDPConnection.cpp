...
{
	UDPServerClient::UDPServerClient(boost::asio::io_service& io_service, std::string host, std::string remote_port, std::string local_port)
      :
    	  host_(host)
	...
	void UDPServerClient::sendRequest(const SensorMessage& message, std::size_t response_size, bool has_response_head_and_body)
	{
    ...
    void UDPServerClient::sendMessage(const boost::asio::mutable_buffer& buffer, std::size_t response_size, bool has_response_head_and_body)
    {
    ...
    void UDPServerClient::getResponse(std::size_t response_size, bool has_response_head_and_body, const boost::system::error_code& error, std::size_t bytes_transferred)
    {
        std::cout << "WRITTEN: " << bytes_transferred << " bytes, trying to receive: " << response_size << " bytes." << std::endl;
        if (!error)
        {
                socket_outgoing_.async_receive_from(boost::asio::buffer(data_, response_size), remote_endpoint_,
                                                  boost::bind(&UDPServerClient::gotResponse, this, has_response_head_and_body,
                                                              boost::asio::placeholders::error,
                                                              boost::asio::placeholders::bytes_transferred));
        }
        else
    ...
    void UDPServerClient::gotResponse(bool has_response_head_and_body, const boost::system::error_code& error, std::size_t bytes_transferred)
    {
    	if(error)
    	{
    ...
    {
        socket_incomming_.async_receive_from(boost::asio::buffer(data_, 2), local_endpoint_,
