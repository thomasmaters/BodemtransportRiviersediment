...
DepthProfileVizualizer::DepthProfileVizualizer(QCustomPlot* custom_plot, QWidget *parent) : DepthProfileVizualizerProxy (parent), custom_plot_(custom_plot)
{
...
void DepthProfileVizualizer::messageReceived(Controller::BottomTransportMessage message)
{
    ...
    for(auto& i: dune_data)
    {
        for (std::size_t j = 1; j < 24; ++j)
        {
            ...
            y.append(i.at(0,0)*std::pow(cur_x,3) + i.at(1,0)*std::pow(cur_x,2) + i.at(2,0)*cur_x + i.at(3,0));
        }
    }
